...
int main() {
	...
	for(auto it=goods.begin();it!=goods.end();it++){
		...
		int max_area=5;
		while(true){
			if(i-change_factor>=0 && i+change_factor<N && j-change_factor>=0 && j+change_factor<M && grid[i-change_factor][j]=='G' && grid[i+change_factor][j]=='G' && grid[i][j-change_factor]=='G' &&  grid[i][j+change_factor]=='G'){
